...
def measure_dark_baseline_96():
    print("\nMeasuring DARK baseline for all 96 photodiodes...")
    ...
    print("Photodiode,DarkRawADC,DarkVoltageV")
    ...
    for column_index in range(12):
        for row_index in range(8):
            ...
            print(
                f"{name},"
                f"{dark_raw_grid[column_index][row_index]},"
                f"{dark_voltage_grid[column_index][row_index]:.6f}"
            )

    return dark_raw_grid, dark_voltage_grid


def scan_96_photodiodes(dark_raw_grid, dark_voltage_grid):
    raw_grid = []
    voltage_grid = []
    light_signal_grid = []
    voltage_signal_grid = []
    ...
    for column_index in range(12):
        ...
        for row_index in range(8):
            light_signal = abs(raw_rows[row_index] - dark_raw_grid[column_index][row_index])
            voltage_signal = abs(voltage_rows[row_index] - dark_voltage_grid[column_index][row_index])
...
def print_96_results(raw_grid, voltage_grid, light_signal_grid, voltage_signal_grid, dark_raw_grid, dark_voltage_grid):
    print("\n96-PHOTODIODE READINGS")
    print("Photodiode,DarkRawADC,RawADC,RawChangeFromDark,LightSignal,DarkVoltageV,VoltageV,LightSignalVoltageV")
...
def run_96_photodiode_test():
    print("96-PHOTODIODE TEST")
    print("This scans 12 columns.")
    print("For each column, it reads 8 photodiodes.")
    print("Total: 96 photodiode positions.")
    print("No light calibration.")
    print("First it measures DARK baseline.")
    print("Then it prints readings every second.")

    countdown("Cover the photodiodes for DARK baseline.", 7)

    dark_raw_grid, dark_voltage_grid = measure_dark_baseline_96()

    print("\nNow 96 photodiode readings will print automatically every second.")

    while True:
        raw_grid, voltage_grid, light_signal_grid, voltage_signal_grid = scan_96_photodiodes(
            dark_raw_grid,
            dark_voltage_grid
        )
